...
export default function InformasiPage() {
  ...
  const { data: informasiList = [], isLoading } = useQuery({
    queryKey: ['admin-informasi', filterBidang],
    queryFn: async () => {
      let query = supabase.from('informasi').select('*').order('created_at', { ascending: false });
  ...
  const togglePublishMutation = useMutation({
    mutationFn: async ({ id, is_published }: { id: string, is_published: boolean }) => {
      const { error } = await supabase.from('informasi').update({ is_published: !is_published }).eq('id', id);
      if (error) throw error;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['admin-informasi'] });
  ...
  const filteredData = informasiList.filter((item: any) => 
  ...
  return (
    <div className="mx-auto max-w-6xl">
      <div className="mb-8 flex flex-col items-start justify-between gap-4 md:flex-row md:items-center">
        <div>
          <h1 className="text-2xl font-bold text-textPrimary md:text-3xl">Manajemen Informasi & Berita</h1>
          <p className="text-textSecondary">Kelola artikel pengumuman atau berita untuk pengunjung website.</p>
        </div>
        <Link
          href="/admin/informasi/baru"
                ...
                filteredData.map((item: any) => (
                  <tr key={item.id} className="border-b border-border transition-colors hover:bg-slate-50 last:border-0">
                    ...
                    <td className="px-6 py-4">
                      ...
                      >
                        {item.is_published ? <CheckCircle size={14} /> : <XCircle size={14} />}
